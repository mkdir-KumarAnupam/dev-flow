...
class DeepWorkWidgetProvider : AppWidgetProvider() {
    ...
    private fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        ...
        views.setTextViewText(R.id.tv_title, "DEEP WORK")
        views.setTextViewText(R.id.tv_subtext, "total tracked")
        ...
        db.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    var value = snapshot.child("deepWork").getValue()?.toString() ?: "--"
                    if ("DeepWork" == "DeepWork") value += "h"
                    if ("DeepWork" == "AvgFlow") value += "%"
                    if ("DeepWork" == "Practice") value += "m"
                    if ("DeepWork" == "Streak") value += "d"
