...
class SolvedWidgetProvider : AppWidgetProvider() {
    ...
    private fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        ...
        views.setTextViewText(R.id.tv_title, "SOLVED")
        views.setTextViewText(R.id.tv_subtext, "problems attempted")
        ...
        db.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    var value = snapshot.child("solved").getValue()?.toString() ?: "--"
                    if ("Solved" == "DeepWork") value += "h"
                    if ("Solved" == "AvgFlow") value += "%"
                    if ("Solved" == "Practice") value += "m"
                    if ("Solved" == "Streak") value += "d"
