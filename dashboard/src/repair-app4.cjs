...
// Find the first instance of the floating nav comment (using substring to avoid encoding issues)
const floatingNavIndex = content.indexOf('FLOATING BOTTOM NAV');
if (floatingNavIndex === -1) {
    console.log('Failed to find floating nav');
    process.exit(1);
}

// Backtrack to the start of that line
const beforeNav = content.substring(0, content.lastIndexOf('\\n', floatingNavIndex));
...
const deployIndex = content.lastIndexOf('Deployments Modal');
if (deployIndex === -1) {
    console.log('Failed to find deploy modal');
    process.exit(1);
}

const afterDeploy = content.substring(content.lastIndexOf('\\n', deployIndex));

const finalContent = beforeNav + '\\n' + freshDock + afterDeploy;

fs.writeFileSync('App.tsx', finalContent, 'utf-8');
console.log('App.tsx perfectly repaired!');
